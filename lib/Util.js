class Util {
   ...
   static async fetch (url, {
      ...
      const responseBody =
         response.headers.get('Content-Type')?.includes('application/json') ?
            await response.json() :
            await response.text(); // If response isn't JSON, return a String instead -- TODO: This may be a poor design
   ...
   static formatNumber(value, {
      ...
      /* Localized to string */
      let localized = number.toLocaleString(locale, options);

      /* Remove negative zero.

      Note: This could be achieved with signDisplay = 'negative' above, however Firefox only
      supports it from version 116. At the time of writing Vite build target is Firefox 114,
      however there does not appear to be a polyfill.

      Firefox 115 support is relatively important because it is the most recent version
      supported on Windows 7.

      Without signDisplay = 'negative', a common case is that a number is rounded to at most
      one decimal, in which case -0.04 rounds to -0. */
